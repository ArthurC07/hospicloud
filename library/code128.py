...
http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/426069 submitted by
Remi Inconnu.
...
Modified by Carlos Flores <cafg10@gmail.com> in order to comply to PEP8 and to
make the code more object oriented and self contained

Code 128 is variable length and a 103 module checksum is added automatically.

Create bar code sample:
   from code128 import Code128
   barcode = Code128("9782212110708")
...
class Code128(object):
    ...
    """Creates a new Code 128 barcode based on the given string"""
    ...
    def __init__(self, code):
        ...
        Stores a string which contains "0" for a white bar, "1" for a black bar
        """
        ...
        for c in range(len(code)):
            ...
            if len(code[c:]) >= 4 and code[c:c + 4].isdigit() \
                and current_charset != self.CharSetC or len(code[c:]) \
                >= 2 and code[c:c + 2].isdigit() and current_charset \
                == self.CharSetC:
                ...
                if current_charset != self.CharSetC:
                    ...
                    if pos:
                        self.bits += \
                            self.ValueEncodings[current_charset['Code C']]
                        total += pos * current_charset['Code C']
                    else:
                        self.bits = \
                            self.ValueEncodings[self.CharSetC['START C']]
                        total = self.CharSetC['START C']
                    current_charset = self.CharSetC
                    pos += 1
            
            elif self.CharSetB.has_key(code[c]) and current_charset \
                != self.CharSetB \
                and not (self.CharSetA.has_key(code[c])
                ...
                if pos:
                    self.bits += \
                        self.ValueEncodings[current_charset['Code B']]
                    total += pos * current_charset['Code B']
                else:
                    self.bits = \
                        self.ValueEncodings[self.CharSetB['START B']]
                    total = self.CharSetB['START B']
                current_charset = self.CharSetB
                pos += 1
            elif self.CharSetA.has_key(code[c]) and current_charset \
                != self.CharSetA \
                and not (self.CharSetB.has_key(code[c])
                ...
                if pos:
                    self.bits += \
                        self.ValueEncodings[current_charset['Code A']]
                    total += pos * current_charset['Code A']
                else:
                    self.bits += \
                        self.ValueEncodings[self.CharSetA['START A']]
            ...
            total += pos * val
            self.bits += self.ValueEncodings[val]
            pos += 1
        
        # Checksum
        checksum = total % 103
        self.bits += self.ValueEncodings[checksum]
        # The stop character
        self.bits += self.ValueEncodings[current_charset['STOP']]
        # Termination bar
        self.bits += '11'
    
    def render(self, height=50, draw_text=False):
        """
        Generate an :class:`Image` object that contains the rendered barcode
        
        """
        # Create a new image
        position = 8
        image = Image.new('1', (len(self.bits) + position, height))
        # Create drawer
        ...
        if draw_text:
            ...
            decode_font_file(courB08_pil, 'courB08.pil')
            decode_font_file(courB08_pbm, 'courB08.pbm')
            # Load font
            font = ImageFont.load(os.path.join(module_dir, 'courB08.pil'))
            draw.text((0, height - 9), self.code, font=font, fill=0)
        
        # Draw the bar codes
        for bit in range(len(self.bits)):
            if self.bits[bit] == '1':
                draw.rectangle(((bit + position, 0), (bit + position,
                               height - 10)), fill=0)
        return image

def decode_font_file(data, filename):
    """ Decode font filename embedded in this script and create filename """

    # If the font filename is missing
    absolute = os.path.join(module_dir, filename)
    if not os.path.exists(absolute):
        # Write font filename
        open(filename, 'wb').write(decompress(decodestring(data)))

def test_object():
    
    """Test the new object interface"""
    
    barcode = Code128('9782212110708')
    image = barcode.render()
    image.save(barcode.code + '.png')
    barcode = Code128('HI345678')
    assert(barcode.bits == '11010010000110001010001100010001010111011110100010110001110001011011000010100100001001101100011101011')
